// onesys-autorecharge-postpay.js (clean rewrite)
...
async function goToTransferPostpay(page) {
  const url = 'https://onesys.onexox.my/ecash_transfer_postpay';
  try {
...
async function clickContinueThenSubmit(page) {
  ...
  while (Date.now() - startTime < 30000) { // wait up to 30s for toast/page update
    const criteria = await page.evaluate(() => {
      ...
      if (bodyText.includes('request to transfer postpay was successfully submitted'))
        detected.push('contains postpay success message');
      ...
      if (bodyText.includes('your request to transfer postpay was successfully submitted'))
      ...
      for (const frame of page.frames()) {
        try {
          const frameCriteria = await frame.evaluate(() => {
            ...
            if (bodyText.includes('request to transfer postpay was successfully submitted'))
              found.push('frame contains postpay success message');
            if (bodyText.includes('your request to transfer postpay was successfully submitted'))
...
async function handleERechargePostpay(page) {
  console.log('ERECHARGE_POSTPAY_START');
  const navigated = await goToTransferPostpay(page);
  if (!navigated) {
    ...
    await clickMenuByHref(page, '/ecash_transfer_postpay');
    await delay(3000);
  }
...
async function main() {
  ...
  if (!session) {
    console.log('AUTO_RECHARGE_POSTPAY_FAILED');
    return;
  }
  const { browser, page } = session;
  try {
    await closePopup(page);
    const result = await handleERechargePostpay(page);
    if (result && result.success) {
      console.log('AUTO_RECHARGE_POSTPAY_SUCCESS');
    } else {
      console.log('AUTO_RECHARGE_POSTPAY_FAILED');
    }
  } catch (err) {
    console.error('AUTO_RECHARGE_POSTPAY_FAILED', err.message);
  } finally {
    await delay(3000);
    await browser.close();
    console.log('AUTORECHARGE_POSTPAY_DONE');
  }
}

main().catch(err => {
  console.error('AUTO_RECHARGE_POSTPAY_FAILED', err);
});
